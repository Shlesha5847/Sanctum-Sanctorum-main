...
def create_loan(db: Session, data: LoanCreate, now: datetime) -> LoanOut:
    ...
    unreturned_loans = list(
        db.scalars(select(Loan).where(Loan.member_id == member.id, Loan.returned_at.is_(None))).all()
    )

    if any(now > l.due_at for l in unreturned_loans):
        raise HTTPException(status_code=409, detail="Member has overdue loans")

    if any(l.book_id == book.id for l in unreturned_loans):
    ...
    if limit is not None and len(unreturned_loans) >= limit:
        raise HTTPException(status_code=409, detail=f"Member reached the loan limit of {limit} for tier '{member.tier}'")

    if book.stock <= 0:
        raise HTTPException(status_code=409, detail=f"Book '{book.title}' is out of stock")
...
def list_member_loans(
    ...
    loans = list(db.scalars(select(Loan).where(Loan.member_id == member_id).order_by(Loan.id.asc())).all())

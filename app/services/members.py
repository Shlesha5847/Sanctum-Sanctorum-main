...
def get_member_stats(db: Session, member_id: int, now: datetime) -> MemberStats:
    ...
    paid_orders = list(
        db.scalars(select(Order).where(Order.member_id == member.id, Order.status == OrderStatus.PAID.value)).all()
    )
    ...
    returned_loans = list(
        db.scalars(select(Loan).where(Loan.member_id == member.id, Loan.returned_at.is_not(None))).all()
    )
